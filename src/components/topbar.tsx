...
export function Topbar({ title, userName }: TopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-white/10 bg-[#080b10]/82 px-4 backdrop-blur-xl sm:px-5 md:px-8 xl:px-10 2xl:px-12">
      <div className="mx-auto flex h-16 w-full max-w-[1560px] items-center justify-between gap-3 sm:h-20 sm:gap-4">
        <div className="flex min-w-0 items-center gap-3">
          <MobileNav />
          <div className="min-w-0">
            <p className="truncate text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-300/75 sm:text-xs sm:tracking-[0.22em]">FinanceHub</p>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          ...
          <form action={logoutAction} className="hidden sm:block">
            <Button type="submit" variant="ghost" size="icon" className="size-11 rounded-full text-slate-400 hover:bg-white/[0.08] hover:text-white" aria-label="Sair">
              <LogOut />
            </Button>
          </form>
        </div>
